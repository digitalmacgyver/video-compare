...
def generate_crossclip_html(all_data, clip_labels):
    ...
    # Scatter datasets (each device x each clip)
    symbols = ["circle", "triangle", "rect", "rectRot", "star", "cross", "crossRot",
               "dash", "line", "rectRounded"]
    scatter_datasets = []
    for dev in device_names:
        for ci, clip in enumerate(clip_labels):
            scatter_datasets.append({
                "label": "{} ({})".format(dev, clip),
                "data": [{"x": all_composites[clip][dev]["tech"],
                          "y": all_composites[clip][dev]["perc"]}],
                "backgroundColor": dev_colors.get(dev, "#888"),
                "borderColor": dev_colors.get(dev, "#888"),
                "pointRadius": 7, "pointHoverRadius": 10,
                "pointStyle": symbols[ci % len(symbols)],
            })
...
<p class="subtitle">
  ...
  Each clip was independently normalized and evaluated on 12 no-reference metrics.
</p>
...
<div class="card">
  ...
  <p class="legend-note">Grouped bar chart showing each device's overall z-score per clip. Devices sorted by average rank.</p>
</div>

<h2>Average Overall Score (Across All Clips)</h2>
<div class="card">
  <div class="chart-container" style="height:{bar_h}px;">
    <canvas id="avgOverall"></canvas>
  </div>
  <p class="legend-note">Mean of overall z-scores across {clip_list}.</p>
</div>

<h2>Technical vs. Perceptual — All Clips</h2>
<div class="card">
  <div class="chart-container" style="height:550px;">
    <canvas id="crossScatter"></canvas>
  </div>
  <p class="legend-note">Each device appears {n_clip} times (once per clip). Same color = same device. Shape distinguishes clips.</p>
</div>
...
<div class="card" style="overflow-x:auto;">
  <table class="heatmap" id="consistencyTable">
    <thead>
      <tr>
        <th>Device</th><th>Avg Overall</th>
        {th_metrics}
      </tr>
    </thead>
    <tbody></tbody>
...
new Chart(document.getElementById('crossOverall'), {{
  ...
  options: {{
    ...
    scales: {{
      ...
      y: {{ grid: {{ color: '#30363d' }}, ticks: {{ color: '#8b949e' }}, title: {{ display: true, text: 'Overall Z-Score', color: '#8b949e' }} }}
    }}
  }}
}});

hbar('avgOverall', {ranked_dev_json}, {avg_scores_json}, {dev_colors_json}, 'Average Overall Z-Score');

new Chart(document.getElementById('crossScatter'), {{
  type: 'scatter',
  data: {{ datasets: {scatter_json} }},
  options: {{
    responsive: true, maintainAspectRatio: false,
    plugins: {{
      legend: {{ display: false }},
      tooltip: {{ callbacks: {{ label: ctx => ctx.dataset.label + ' (tech: ' + ctx.parsed.x.toFixed(2) + ', perc: ' + ctx.parsed.y.toFixed(2) + ')' }} }}
    }},
    scales: {{
      x: {{ grid: {{ color: '#30363d' }}, ticks: {{ color: '#8b949e' }}, title: {{ display: true, text: 'Technical Quality Z-Score \\u2192', color: '#58a6ff' }} }},
      y: {{ grid: {{ color: '#30363d' }}, ticks: {{ color: '#8b949e' }}, title: {{ display: true, text: 'Perceptual Quality Z-Score \\u2192', color: '#d2a8ff' }} }}
    }}
  }}
}});
...
consistData.forEach(row => {{
  ...
  td.textContent = row.avg_overall.toFixed(2);
  const n = Math.max(-1, Math.min(1, row.avg_overall / 3));
...
document.querySelectorAll('.heatmap th').forEach((th,colIdx)=>{{
  th.addEventListener('click',()=>{{
    ...
    rows.forEach(r=>tbody.appendChild(r));
        ...
        th_metrics="".join(
            '<th class="{}">{}</th>'.format(
                "th-tech" if METRIC_INFO[k][3] == "tech" else "th-perc",
                METRIC_INFO[k][0]
            ) for k in ALL_KEYS
        ),
        ...
        scatter_json=json.dumps(scatter_datasets),
